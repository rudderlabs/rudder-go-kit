...
func TestUpload(t *testing.T) {
	...
	mockSFTPClient.EXPECT().Open(gomock.Any(), gomock.Any()).Return(&nopReadWriteCloser{remoteBuf}, nil)
...
func TestDownload(t *testing.T) {
	...
	remoteReader := &nopReadWriteCloser{remoteBuf}

	mockSFTPClient := mock_sftp.NewMockClient(ctrl)
	mockSFTPClient.EXPECT().Open(gomock.Any(), gomock.Any()).Return(remoteReader, nil)
