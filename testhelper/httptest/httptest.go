// Package httptest provides similar functionality to the net/http/httptest package, but with some additional features:
//
//   - It allows you to listen to all interfaces, not just localhost.
//   - For TLS servers, it allows you to specify the host name to use in the certificate.
package httptest
...
func (s *Server) Start() {
	...
	s.URL = "http://" + net.JoinHostPort("127.0.0.1", port)
	s.wrap()
	s.goServe()
	if serveFlag != "" {
...
func (s *Server) StartTLS(host string) {
	if s.URL != "" {
		panic("Server already started")
	}
	if s.client == nil {
	...
	pem, key := generateCert(strings.Join(hosts, ","))
	...
	s.URL = "https://" + net.JoinHostPort(listenerhost, port)
	s.wrap()
	s.goServe()
}
...
// The caller should call Close when finished, to shut it down.
func NewTLSServer(host string, handler http.Handler) *Server {
	ts := NewUnstartedServer(handler)
	ts.StartTLS(host)
	return ts
}
...
func generateCert(host string) (cert, key []byte) {
