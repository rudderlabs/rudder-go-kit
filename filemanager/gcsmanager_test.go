...
func TestGCSManagerConcurrentDelete(t *testing.T) {
	...
	for _, concurrency := range concurrencyLevels {
		t.Run(fmt.Sprintf("concurrency_%d", concurrency), func(t *testing.T) {
			...
			// Reset concurrency tracking for each test
			mu.Lock()
			maxConcurrent = 0
			currentConcurrent = 0
			mu.Unlock()

			// Create a new bucket for each test
			bucketName := fmt.Sprintf("test-bucket-%d", concurrency)
			server.CreateBucket(bucketName)

			prefix := fmt.Sprintf("test-prefix-%d", concurrency)

			// Configure GCS manager with specific concurrency
			conf := map[string]interface{}{
